...
impl GithubClient {
    ...
    pub async fn create_or_update_file(&self, path: &str, content: &str, message: &str, sha: Option<&str>) -> Result<GithubCreateUpdateResponse> {
        let url = format!(
            "https://api.github.com/repos/{}/{}/contents/{}",
            self.owner, self.repo, path
        );
    ...
    pub fn decode_file_content(&self, file_content: &GithubFileContent) -> Result<String> {
        ...
        String::from_utf8(decoded)
            .map_err(|e| anyhow!("Invalid UTF-8 content: {}", e))
    }
}